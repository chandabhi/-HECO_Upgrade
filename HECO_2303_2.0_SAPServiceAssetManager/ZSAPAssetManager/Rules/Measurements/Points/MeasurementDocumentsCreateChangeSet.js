...
export default function MeasurementDocumentsCreateChangeSet(context) {
    ...
    return fdcHelper.run((sectionBinding, section) => {
        ...
        // if (startVal && endVal) {
        //     SetStartPointEndPointLength(startVal, endVal, controlDict.Length, controlDict.Length);
        // }
...
function recordReadings(context) {
    ...
    return fdcHelper.run(async (sectionBinding, section) => {
        ...
        //sectionBinding should be a measuring point object.
        if (sectionBinding['@odata.type'] === '#sap_mobile.MyWorkOrderTool') { //sectionBinding is a PRT object
            sectionBinding = sectionBinding.PRTPoint; //get measuring point object from PRT
        }
        ...
        // from operation page ShortText property can be null or undefined
        ...
        if (reading || valuationCode) {
            ...
            let properties = {
                ...
                'OrderObjNum': (() => {
                    ...
                    try {
                        ...
                        } else if (context.binding['@odata.type'] === '#sap_mobile.MyWorkOrderOperation') { //Scenario when take reading action is clicked from PRT screen.
                            if(context.binding.WOHeader && context.binding.WOHeader.ObjectKey){
                                return context.binding.WOHeader.ObjectKey;
                            }
                            return '';
                        } else {
                            if (common.isDefined(clientData.MeasuringPointData[sectionBinding.Point].OrderId)) {
                        ...
                        Logger.error(context.getGlobalDefinition('/SAPAssetManager/Globals/Logs/CategoryMeasurementDocuments.global').getValue(),`MeasurementDocumentsCreateChangeSet(context) OrderObjNum error: ${err}`);
                        return '';
                    }
                })(),
                'OperationObjNum': (() => {
                    ...
                    try {
                        if (Object.prototype.hasOwnProperty.call(context.binding, 'Point')) {
                            ...
                            if(context.binding.ObjectKey){
                                return context.binding.ObjectKey;
                            }
                            return '';
                        } else {
                        ...
                        Logger.error(context.getGlobalDefinition('/SAPAssetManager/Globals/Logs/CategoryMeasurementDocuments.global').getValue(),`MeasurementDocumentsCreateChangeSet(context) OperationObjNum error: ${err}`);
                        return '';
                    }
                })(),
            };

            //You can have a measuring point that takes both a reading value and a valuation code.
            if (reading) {
            ...
            /**
             * You can have a measuring point that takes only valuation code. Remove ReadingValue & RecordedValue properties.
             * Otherwise, actions MeasurementDocumentCreateForChangeSet and MeasurementDocumentUpdate will fail.
             */
            if (valuationCode && !reading) {
                delete properties.ReadingValue;
                delete properties.RecordedValue;
            }
