...
export default async function OperationChangeStatusOptions(context, WOPageFlag = false) {
    ...
    if (WOPageFlag && context.binding['@odata.type'] === "#sap_mobile.MyWorkOrderHeader") {
        let { Operations } = context.binding;
        OrderType = context.binding.OrderType;
        if ((Operations && Operations.length === 0) || !Operations) {
            return Promise.resolve();
        }
        var bindingOriginal = context.binding;
        let operation = await context.read("/SAPAssetManager/Services/AssetManager.service", Operations[0]['@odata.readLink'], [], "$expand=OperationMobileStatus_Nav");
        operationBinding = operation._array[0]
        context._context.binding = operationBinding; // replace binding with action binding so that we can use WorkOrderChangeStatusOptions before we navigated to the page
    } else if (context.binding['@odata.type'] === "#sap_mobile.MyWorkOrderOperation") {
        if(context.binding.WOHeader&&context.binding.WOHeader.OrderType){
            OrderType = context.binding.WOHeader.OrderType;
        }
        let operation = await context.read("/SAPAssetManager/Services/AssetManager.service", context.binding['@odata.readLink'], [], "$expand=OperationMobileStatus_Nav");
        operationBinding = operation._array[0];
    }
    ...
    if (assnType === '1' || assnType === '5' || assnType === '7' || assnType === '8') {
        ...
        return Promise.all([libOpMobile.isAnyOperationStarted(context), roleCheck(), PhaseLibrary.isPhaseModelActiveInDataObject(context, context.binding)]).then(async checks => {
            ...
            if (libVal.evalIsEmpty(operationBinding?.OperationMobileStatus_Nav||context.binding.OperationMobileStatus_Nav)) {
                return Promise.resolve([]);
            }
            ...
            const mobileStatus = context.binding.OperationMobileStatus_Nav.MobileStatus|| context.binding.OperationMobileStatus_Nav.MobileStatus;
            const statusIsLocal = context.binding.OperationMobileStatus_Nav['@sap.isLocal']|| context.binding.OperationMobileStatus_Nav['@sap.isLocal'];
            const isClockedIn = (libClock.isBusinessObjectClockedIn(context) && libClock.allowClockInOverride(context, mobileStatus));
            // sap note changes for order dependency
            let queryOptions = `$expand=NextOverallStatusCfg_Nav&$filter=UserPersona eq \'MAINTENANCE_TECHNICIAN\' and ToEAMOverallStatusProfile eq \'\'  and OverallStatusCfg_Nav/MobileStatus eq '${mobileStatus}' and OverallStatusCfg_Nav/ObjectType eq \'WO_OPERATION\'`;
                ...
                return libSuper.checkReviewRequired(context, context.binding).then(review => {
                    return context.read('/SAPAssetManager/Services/AssetManager.service', `${context.binding['@odata.readLink']}/OperationMobileStatus_Nav`, [], '$expand=OverallStatusCfg_Nav').then(rollback => {
                        ...
                        return phaseModelCheck.then(stopKeys => {
                            //sap note changes for order dependency
                            return context.read('/SAPAssetManager/Services/AssetManager.service', 'EAMOverallStatusSeqs', [], queryOptions).then(codes => {
                                let popoverItems = [];
                                ...
                                codes.forEach(element => {
                                    ...
                                    if(statusElement.TransitionTextKey&&statusElement.TransitionTextKey == "enroute"&&["ED01", "ED02", "ED03", "ED04", "ED05", "ED06"].includes(OrderType)){
                                        return true;
                                    }
                                        ...
                                        if (element.RoleType === userRoleType && stopKeys.findIndex(stopKey => stopKey.ProcessPhase === statusElement.Phase && stopKey.ProcessSubphase === statusElement.Subphase) !== -1) {
                                            ...
                                            } else if (statusElement.MobileStatus === TRANSFER && (element.RoleType === userRoleType || personaLib.isFieldServiceTechnician(context) || !libSuper.isSupervisorFeatureEnabled(context))&&(OrderType&&OrderType!="TC01")) {
                                ...
                                if (WOPageFlag) { // E_630
                                    // Did changes to change binding back to orginal for header level change
                                    context._context.binding = bindingOriginal ? bindingOriginal : context.binding;
                                }
