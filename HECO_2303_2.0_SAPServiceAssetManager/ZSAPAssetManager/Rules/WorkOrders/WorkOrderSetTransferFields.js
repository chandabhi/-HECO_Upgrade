...
export default function WorkOrderSetTransferFields(context) {
    ...
    if (context.evaluateTargetPathForAPI('#Page:-Previous').getClientData().FromErrorArchive || context.evaluateTargetPathForAPI('#Page:-Previous').getClientData().ErrorObject) {
        ...
        // if(context.binding['@odata.type']==="#sap_mobile.MyWorkOrderHeader"&&context.binding.OrderType === "TC01"){
        //     bindingOrginals = context.binding;
        //     binding = context.binding.Operations[0];
        //     context._context.binding = context.binding.Operations[0];
        // }
        ...
        rulePromises.push(context.read('/SAPAssetManager/Services/AssetManager.service', `${context.binding['@odata.readLink']}/${mobileStatusNav}/OverallStatusCfg_Nav/OverallStatusSeq_Nav`, [], `$filter=NextOverallStatusCfg_Nav/MobileStatus eq '${TRANSFER}'&$expand=NextOverallStatusCfg_Nav`).then(result => {
            if(context.binding['@odata.type']==="#sap_mobile.MyWorkOrderHeader"&&context.binding.OrderType === "TC01"){
                return context.executeAction(mobileStatusOverride(context, result.getItem(0).NextOverallStatusCfg_Nav, mobileStatusNav, postRule)).then(() => {
                    postRule = '/SAPAssetManager/Rules/MobileStatus/OperationMobileStatusPostUpdate.js'
        ...
        switch (assnType) {
            ...
            if(context.binding['@odata.type']==="#sap_mobile.MyWorkOrderHeader"&&context.binding.OrderType === "TC01"){
                readlink = binding.Operations[0]['@odata.readLink']
            }
                context.getClientData().EmployeeTo = ruleData.EmployeeTo;
                context.getClientData().OperationReadLink = readlink;
        ...
        return Promise.all(rulePromises).then(() => {
            // if(context.binding['@odata.type']==="#sap_mobile.MyWorkOrderOperation"&&context.binding.OrderType === "TC01"&&bindingOrginals){
            //     context._context.binding = bindingOrginals;
            // }
            // if(context.binding['@odata.type']==="#sap_mobile.MyWorkOrderHeader"&&context.binding.OrderType === "TC01"){
            //     return OpsMobileStat(context).then(() => {
