            ...
            case 'MainWorkCenterLstPkr':
                {
                    ...
                    result = "$orderby=ExternalWorkCenterId&$filter=PlantId eq '" + plant + "' and ZUsedForMobileWorkOrderCreate eq 'X'";
                    break;
                }
            default:
                break;
    ...
    static CreateUpdateOnCommit(pageProxy) {
        ...
        if (onCreate) {
            return libWo.isServiceOrderCreateUpdate(pageProxy).then(isServiceOrder => {
                ...
                //HECO
                let sensitiveValue = formCellContainer.getControl('ZDocumentSensitiveSwitch').getValue();
                libCommon.setStateVariable(pageProxy, 'ZDocumentIsSensitive', sensitiveValue);
                //ENDHECO
