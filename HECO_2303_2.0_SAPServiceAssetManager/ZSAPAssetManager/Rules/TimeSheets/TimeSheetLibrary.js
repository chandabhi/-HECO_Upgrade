...
export class TimeSheetEventLibrary {
    ...
    static TimeSheetEntryCreateUpdateValidation(pageClientAPI) {
        ...
        //HECO Add Wage Type Field
        libCom.getFieldValue(pageClientAPI, 'ZWageTypeLstPkr', '', dict, true);
    ...
    static ValidateWorkorderRequired(pageClientAPI, dict) {
        if (dict.ZWageTypeLstPkr == '' || dict.ZWageTypeLstPkr == '1104') {
            if (!libVal.evalIsEmpty(dict.RecOrderLstPkr)) {
                return Promise.resolve(true);
            } else {
                return pageClientAPI.executeAction('/SAPAssetManager/Actions/TimeSheets/ErrorDialogs/WorkOrderRequiredError.action').then(function() {
    ...
    static ValidateAbsAttOrActivityType(pageClientAPI, dict) {
        ...
        let wageType;
        let listPickerValue = pageClientAPI.getPageProxy().getControl('FormCellContainer').getControl('ZWageTypeLstPkr').getValue();
        if(Array.isArray(listPickerValue) && listPickerValue.length == 1) {
            wageType = listPickerValue[0].ReturnValue;
        }
        
        if (wageType) { //HECO Wage types don't allow use of attendance codes
            return Promise.resolve(true);
        } else if (!libVal.evalIsEmpty(dict.AbsAttLstPkr))  //HECO removed !libVal.evalIsEmpty(dict.ActivityTypeLstPkr) || 
        {
            return Promise.resolve(true);
        } else {
