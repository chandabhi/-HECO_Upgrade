...
export default function TimeSheetEntryCreateUpdateRecOrderUpdate(controlProxy) {
    ...
    //HECO
    //Removing control key requirement to allow time entry on ED06 work
    //CATS entry will automaticly be adjusted to billable entry in backend since those operations are not assigned
    ...
    if (libVal.evalIsEmpty(selection)) { //No order, so disable and empty op and sub-op pickers
        ...
        let filter = '$orderby=OperationNo asc'; //Show all operations
