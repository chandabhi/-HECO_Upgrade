        ...
        if (libCommon.isDefined(mobileStatusObject)) {
            ...
            return libSuper.checkReviewRequired(context, target).then(review => {
                ...
                return libClock.reloadUserTimeEntries(context, false, false, mobileInstance).then(() => {
                    return libClock.clockOutBusinessObject(context, mobileInstance).then(async () => {//sap note changes for 3365886
                        ...
                        let completeValue = await context.read('/SAPAssetManager/Services/AssetManager.service', 'EAMOverallStatusConfigs', [], `$filter=MobileStatus eq '${completeMobileStatus}' and ObjectType eq '${eamObjType}' and EAMOverallStatusProfile eq '${mobileStatusObject.EAMOverallStatusProfile}'`);//sap note for changes 3365886
                        const properties = {
                            ...
                            'EAMOverallStatusProfile': completeValue.getItem(0).EAMOverallStatusProfile,//sap note changes for 3365886
                            'Status': completeValue.getItem(0).Status,//sap note changes for 3365886
                        };
                        ...
                        return context.executeAction({
                            'Name': '/SAPAssetManager/Actions/Confirmations/MobileStatusSetComplete.action', 'Properties': {
                                ...
                                //removed updatelinks as a part sap note changes for 3365886
                                'Headers': {
